...
class SeparationLogicChecker:
    ...
    def _check_let(self, stmt: LetStmt, func: PureFunc | TaskFunc) -> None:
        ...
        if stmt.value:
            ...
            if self._is_allocation(stmt.value):
                is_uninit = self._is_uninit_alloc(stmt.value)
                self.state.heap[stmt.name] = HeapCell(
                    name=stmt.name,
                    state=HeapCellState.ALLOCATED,
                    allocated_at=loc,
                    is_nullable=False,
                    is_initialized=not is_uninit,
                    alloc_size=self._get_alloc_size(stmt.value),
                )
            ...
            self._check_uninit_read(stmt.value, func, loc)
    ...
    def _check_uninit_read(self, expr: Expr, func: PureFunc | TaskFunc,
                           loc: SourceLocation) -> None:
        """Check if an expression reads from uninitialized memory."""
        if isinstance(expr, Identifier):
            name = expr.name
            if name in self.state.heap:
                cell = self.state.heap[name]
                if not cell.is_initialized and cell.state == HeapCellState.ALLOCATED:
                    self.errors.append(contract_error(
                        precondition=f"Uninitialized memory read: '{name}' was allocated but never written to",
                        failing_values={"variable": name, "engine": "Separation Logic"},
                        function_signature=f"{func.name}",
                        location=loc,
                    ))
        elif isinstance(expr, FieldAccess):
            self._check_uninit_read(expr.obj, func, loc)
        elif isinstance(expr, BinaryOp):
            self._check_uninit_read(expr.left, func, loc)
            self._check_uninit_read(expr.right, func, loc)
        elif isinstance(expr, FunctionCall):
            for arg in expr.args:
                self._check_uninit_read(arg, func, loc)
        elif isinstance(expr, MethodCall):
            self._check_uninit_read(expr.obj, func, loc)
            for arg in expr.args:
                self._check_uninit_read(arg, func, loc)

    def _check_buffer_overflow(self, expr: Expr, func: PureFunc | TaskFunc,
                                loc: SourceLocation) -> None:
        """Check for potential buffer overflow on array/buffer writes."""
        if isinstance(expr, FunctionCall) and isinstance(expr.callee, Identifier):
            ...
            if fname in ("memcpy", "memmove", "strcpy", "strncpy", "strcat",
                        "sprintf", "snprintf", "write", "fwrite", "copy"):
                # Check if destination has known size
                if expr.args and isinstance(expr.args[0], Identifier):
                    dest = expr.args[0].name
                    if dest in self.state.heap:
                        cell = self.state.heap[dest]
                        if cell.alloc_size is not None:
                            # If we have a size argument, check for overflow
                            if len(expr.args) >= 3:
                                size_arg = expr.args[2]
                                if isinstance(size_arg, IntLiteral):
                                    if size_arg.value > cell.alloc_size:
                                        self.errors.append(contract_error(
                                            precondition=(
                                                f"Buffer overflow: writing {size_arg.value} bytes "
                                                f"to '{dest}' which has {cell.alloc_size} bytes allocated"
                                            ),
                                            failing_values={
                                                "variable": dest,
                                                "write_size": size_arg.value,
                                                "buffer_size": cell.alloc_size,
            ...
            # Unsafe string functions without bounds
            if fname in ("strcpy", "strcat", "gets", "sprintf"):
                self.errors.append(contract_error(
                    precondition=(
                        f"Unsafe buffer operation: '{fname}' does not check bounds — "
                        f"use '{fname.replace('str', 'strn').replace('sprintf', 'snprintf').replace('gets', 'fgets')}' instead"
                    ),
                    failing_values={
                        "function": fname,
                        "engine": "Separation Logic",
    ...
    def _is_uninit_alloc(self, expr: Expr) -> bool:
        """Check if allocation is uninitialized (malloc vs calloc)."""
        if isinstance(expr, FunctionCall) and isinstance(expr.callee, Identifier):
            return expr.callee.name.lower() in ("malloc", "alloc", "allocate", "realloc")
        return False

    def _get_alloc_size(self, expr: Expr) -> Optional[int]:
        """Try to extract allocation size from call arguments."""
        if isinstance(expr, FunctionCall) and expr.args:
            if isinstance(expr.args[0], IntLiteral):
...
def check_separation_logic(program: Program) -> List[AeonError]:
    ...
    - Buffer overflow detection on unsafe operations
    - Unsafe buffer function usage (strcpy, gets, etc.)
    """
