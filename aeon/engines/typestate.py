...
def _analyze_function_typestate(func, errors: List[AeonError]) -> None:
    ...
    def _check_expr(expr: Expr) -> None:
        if isinstance(expr, MethodCall):
            ...
            name = expr.name if hasattr(expr, 'name') and isinstance(expr.name, str) else (
                expr.callee.name if hasattr(expr, 'callee') and hasattr(expr.callee, 'name') else ""
            )
    ...
    def _scan_stmt(stmt: Statement) -> None:
        if isinstance(stmt, LetStmt):
            ...
            if isinstance(stmt.value, FunctionCall):
                call_name = stmt.value.name if hasattr(stmt.value, 'name') and isinstance(stmt.value.name, str) else (
                    stmt.value.callee.name if hasattr(stmt.value, 'callee') and hasattr(stmt.value.callee, 'name') else ""
                )
