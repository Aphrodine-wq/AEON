...
def abstract_interpret_with_trace(program: Program, ctx=None):
    """Like abstract_interpret but also returns per-function abstract states.

    Parameters
    ----------
    program : Program
        The AEON program to analyze.
    ctx : VerificationContext, optional
        If provided, proven interval/sign facts are recorded into the context
        so downstream engines can use them as hypotheses.

    Returns
    -------
    (List[AeonError], Dict[str, AbstractState])
        Errors found, and a mapping from function name to its exit AbstractState.
    """
    interpreter = AbstractInterpreter()
    errors = interpreter.analyze_program(program)

    if ctx is not None:
        for func_name, state in interpreter.function_summaries.items():
            for var, interval in state.intervals.items():
                ctx.record_interval(
                    var=var,
                    lo=interval.lo,
                    hi=interval.hi,
    ...
    return errors, interpreter.function_summaries


# ---------------------------------------------------------------------------
# Abstract Domain Inspector — for aeon abstract-trace
# ---------------------------------------------------------------------------

class AbstractDomainInspector:
    """Runs abstract interpretation and captures per-statement abstract states.

    Unlike the standard AbstractInterpreter (which only keeps the exit state),
    this inspector records the abstract state at EVERY program point, enabling
    step-by-step visualization of the fixpoint computation.

    This is the engine behind ``aeon abstract-trace``.
    """

    def __init__(self):
        self.trace: Dict[str, List[Dict[str, Any]]] = {}  # func -> list of steps
        self.errors: List[AeonError] = []

    def inspect_program(self, program: Program) -> Dict[str, List[Dict[str, Any]]]:
        """Inspect all functions and return per-function abstract traces."""
        self.trace = {}
        self.errors = []
        ...
        for func in functions:
            self.trace[func.name] = self._inspect_function(func)

        return self.trace

    def _inspect_function(self, func) -> List[Dict[str, Any]]:
        """Return a list of {stmt, state_before, state_after} dicts."""
        steps = []
        state = AbstractState()
        ...
        for req in func.requires:
            ...
            state = interp._apply_condition(state, req.expr, positive=True)

        steps.append({
            "point": "entry",
            "stmt": f"(parameters: {', '.join(p.name for p in func.params)})",
            "state": self._state_to_dict(state),
            "note": "Initial state after binding parameters and requires clauses",
        })

        interp = AbstractInterpreter()
        for i, stmt in enumerate(func.body):
            state_before = state.copy()
            state = interp._analyze_statement(stmt, state)
            steps.append({
                "point": f"stmt_{i}",
                "stmt": self._stmt_repr(stmt),
                "state_before": self._state_to_dict(state_before),
                "state_after": self._state_to_dict(state),
                "note": self._state_change_note(state_before, state),
            })
        ...
        steps.append({
            ...
            "stmt": "(function exit)",
            "state": self._state_to_dict(state),
            "note": "Final abstract state at function exit",
        })

        return steps

    def _state_to_dict(self, state: AbstractState) -> Dict[str, Any]:
        if state.is_bottom:
            return {"_bottom": True}
        result: Dict[str, Any] = {}
        for var in sorted(set(state.intervals) | set(state.signs)):
            iv = state.intervals.get(var)
            sg = state.signs.get(var)
            cg = state.congruences.get(var)
            entry: Dict[str, Any] = {}
            if iv is not None:
                entry["interval"] = str(iv)
            if sg is not None:
                entry["sign"] = str(sg)
            if cg is not None and not cg.is_top():
                entry["congruence"] = str(cg)
            result[var] = entry
        return result

    def _state_change_note(self, before: AbstractState, after: AbstractState) -> str:
        changed = []
        all_vars = set(before.intervals) | set(after.intervals)
        for v in sorted(all_vars):
            b = before.intervals.get(v)
            a = after.intervals.get(v)
            if str(b) != str(a):
                changed.append(f"{v}: {b} → {a}")
        if not changed:
            return "no change"
        return "; ".join(changed)

    @staticmethod
    def _stmt_repr(stmt) -> str:
        """Short human-readable representation of a statement."""
        from aeon.ast_nodes import (
            LetStmt, AssignStmt, ReturnStmt, IfStmt, WhileStmt, ExprStmt,
        )
        if isinstance(stmt, LetStmt):
            return f"let {stmt.name} = ..."
        if isinstance(stmt, AssignStmt):
            target = getattr(stmt.target, "name", str(stmt.target))
            return f"{target} = ..."
        if isinstance(stmt, ReturnStmt):
            return "return ..."
        if isinstance(stmt, IfStmt):
...
def inspect_abstract_domains(program: Program) -> Dict[str, List[Dict[str, Any]]]:
    """Run the abstract domain inspector and return per-function traces.

    This is the public entry point for ``aeon abstract-trace``.
    """
    inspector = AbstractDomainInspector()
    return inspector.inspect_program(program)
